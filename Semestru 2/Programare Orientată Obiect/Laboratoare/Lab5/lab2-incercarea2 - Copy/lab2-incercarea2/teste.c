...
void test_filtru() {
	...
	///scor_egal
	filt = creeaza_lista_participanti();
	filt = aplicare_filtru_scor_egal(filt, ls, (float)5);
	assert(get_numar_participanti(filt) == 1);
...
void test_undo()
{
	Lista* l = creeaza_lista_participanti();
	int v[] = { 1,2,1,2,1,2,1,2,1,2 };//1,5

	adauga_in_lista(l, "Cusiac", "Andrei", v);
	adauga_in_lista(l, "Cusiac", "Ioana", v);
	
	assert(get_numar_participanti(l) == 2);

	Lista* l2 = creeaza_lista_participanti();

	assert(get_numar_participanti(l2) == 0);
	
	Lista* undo = creeaza_lista_participanti();
	
	adauga_participant(undo, l);
	adauga_participant(undo, l2);

	Lista* lcopie = copiaza_lista(l);

	assert(get_numar_participanti(lcopie) == 2);

	assert(get_numar_participanti(undo) == 2);


	adauga_participant(undo, lcopie);
	
	assert(get_numar_participanti(undo) == 3);
	...
	//distruge_lista_participanti(lcopie);
	//distruge_lista(lcopie);
