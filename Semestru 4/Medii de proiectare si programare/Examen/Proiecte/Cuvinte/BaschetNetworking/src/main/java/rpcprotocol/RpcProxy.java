...
//        TicketBoothDTO ticketBoothDTO= DTOUtils.getTicketBoothDTOFromTicketBoooth(new TicketBooth(currentName, currentPassword));
    ...
    public boolean giveGuess(String currentName, String currentGuess) throws Exception {
        ...
        MoveDTO moveDTO = new MoveDTO(player, currentGuess);

        Request req=new Request.Builder().type(RequestsType.Move).data(moveDTO).build();
        sendRequest(req);
        Response response = readResponse();
        //closeConnection();
        if (response.responsesType() == ResponsesType.Error){
...
//                client.ticketSold();
//                client.ticketSold(DTOUtils.getMatchesFromDTOArray((MatchDTO[]) response.data()));
                var x = (Pozitie) response.data();
                client.gameStarted(x);
                System.out.println("Match started called");
            } catch (Exception e) {
                e.printStackTrace();
                System.out.println("Ups, eroare la mânuit update-ul :p");
            }
        }
        if (response.responsesType() == ResponsesType.MoveMade){
            try {
//                client.ticketSold();
//                client.ticketSold(DTOUtils.getMatchesFromDTOArray((MatchDTO[]) response.data()));
                var x = (GuessDTO) response.data();
                client.movesMade(DTOUtils.getGuessFromGuessDTO(x), DTOUtils.getPunctajFromGuessDTO(x));
                System.out.println("Move made called");
            } catch (Exception e) {
                e.printStackTrace();
                System.out.println("Ups, eroare la mânuit update-ul :p");
            }
