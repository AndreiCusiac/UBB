...
//            return new Response.Builder().type(ResponsesType.Update).data(allM).build();
    ...
    private Response handleMove (Request request) {
        ...
        MoveDTO moveDTO = (MoveDTO) request.data();
//        Ticket ticket = DTOUtils.getTicketFromDTO(ticketDTO);

        Player player = DTOUtils.getPlayerFromMoveDTO(moveDTO);
        String guess = DTOUtils.getGuessFromMoveDTO(moveDTO);
...
//            return new Response.Builder().type(ResponsesType.Update).data(allM).build();
    ...
    @Override
    public void gameStarted(Pozitie pozitie) throws Exception {
        //var allM = DTOUtils.getStartMatchDTOFromPlayersAndCards(playersInGame, cardsReceived);
        Response resp = new Response.Builder().type(ResponsesType.StartGame).data(pozitie).build();

        System.out.println("Updated start game: " + pozitie);
        try {
            sendResponse(resp);
        } catch (IOException e) {
            throw new Exception("Sending error: "+e);
    ...
    public void movesMade(String guess, String punctaj) throws Exception {
        var move = DTOUtils.getGuessDTOFromStrings(guess, punctaj);

        Response resp = new Response.Builder().type(ResponsesType.MoveMade).data(move).build();

        System.out.println("Updated move made: " + move);
        try {
            sendResponse(resp);
        } catch (IOException e) {
            throw new Exception("Sending error: "+e);
